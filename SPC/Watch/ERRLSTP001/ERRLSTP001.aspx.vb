...
#Region "定数定義"
    ...
    '=　定数定義
    '============================================================================================================================
    Private Const M_DISP_ID = P_FUN_ERR & P_SCR_LST & P_PAGE & "001"

    '次画面ファイルパス
    Const M_NEXT_DISP_PATH = "~/" & P_WAT & "/" &
            P_FUN_ERR & P_SCR_SEL & P_PAGE & "001" & "/" &
            P_FUN_ERR & P_SCR_SEL & P_PAGE & "001.aspx"

    Const M_DISP_ERRLSTP001_ID = P_FUN_ERR & P_SCR_LST & P_PAGE & "001"
    ...
    Protected Sub grvList_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles grvList.RowCommand
        ...
        Select Case e.CommandName
            ...
            Case "btnUpdate"    '更新
                ...
                ''★ロック対象テーブル名の登録
                'arTable_Name.Insert(0, "D51_NOREPLY")

                ''★ロックテーブルキー項目の登録(D51_NOREPLY)
        ...
        strKeyList.Add(CType(rowData.FindControl("管理番号"), Label).Text)
        strKeyList.Add(CType(rowData.FindControl("調査年月日"), Label).Text)
        strKeyList.Add(CType(rowData.FindControl("調査日時"), Label).Text)
    ...
    Private Sub ms_GetSearchData(ByVal DateFrom As String _
        ...
        If clsDataConnect.pfOpen_Database(conDB) Then
            ...
            Catch ex As Exception
                ...
                'システムエラー
                psMesBox(Me, "00004", ClsComVer.E_Mタイプ.エラー, ClsComVer.E_S実行.描画後)
                '--------------------------------
                '2014/04/14 星野　ここから
                '--------------------------------
                'ログ出力
